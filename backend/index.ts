...
// POST /api/create-lists
app.post('/api/create-lists', authMiddleware, async (req, res) => {
...
app.post('/api/create-items', authMiddleware, async (req, res) => {
...
app.patch('/api/items/:id', authMiddleware, async (req, res) => {
  const { id } = req.params;
  const { title, notes, done, position } = req.body ?? {};
